...
class SimpleWebSocketServer(object):
   def __init__(self, host, port, websocketclass, selectInterval = 0.1):
      ...
      self.serversocket.listen(25) #todo currently 25 gateways can be connected
   ...
   def _constructWebSocket(self, agent, sock, address):
      return self.websocketclass(self, agent, sock, address)
      ...
      for ready in rList:
         if ready == self.serversocket:
            try:
               ...
               self.connections[fileno] = self._constructWebSocket(agent,newsock, address)
   ...
   def serveforever(self,agent):
      while True:
         self.serveonce(agent)
...
class SimpleSSLWebSocketServer(SimpleWebSocketServer):
   ...
   def _constructWebSocket(self, agent,sock, address):
      ws = self.websocketclass(agent,self,sock, address)
      ws.usingssl = True
      return ws

   def serveforever(self,agent):
      super(SimpleSSLWebSocketServer, self).serveforever(agent)
